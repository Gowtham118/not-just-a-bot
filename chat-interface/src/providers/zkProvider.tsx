...
type ZkContext = {
  wallet: Wallet;
  async(toAddress: string, amount: string, chain: number): Promise<string>;
};
...
export const ZkContext = createContext({
  ...
  generateUserOps: async (
    toAddress: string,
    amount: string,
    chain: number
  ): Promise<string> => {
    return toAddress + amount + chain;
  },
});
...
export const ZkProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const generateUserOps = async (
    ...
    if (!wallet.address || !wallet.balance)
      throw new Error("Address or balance is missing");
    ...
    const data = ethers.utils.defaultAbiCoder.encode(
      ...
      [entrypoint.address, import.meta.env.VITE_SYSTEM_ADDRESS, p?.pwdhash]
    );
    ...
    const userOps = {
      ...
      callData: new ethers.utils.Interface(ZK_ELLASTIC_ABI).encodeFunctionData(
        "execute",
        [toAddress, ethers.utils.parseEther(amount), "0x"] //check again
      ),
      callGasLimit: 40000,
      verificationGasLimit: 1503500,
      maxFeePerGas: Number(
      ...
      if (isAuthenticated) {
        try {
          const response = await axios.post(
            import.meta.env.VITE_BACKEND_URL + "/wallet" + "/get-wallet",
            {
              Wallet_type: ZK_ELASTIC_WALLET,
          ...
          const response = await axios.post(
            import.meta.env.VITE_BACKEND_URL + "/wallet" + "/create-wallet",
            {
              Wallet_type: ZK_ELASTIC_WALLET,
              Value: (await getProof(
                password,
